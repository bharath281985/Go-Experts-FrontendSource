    ...
    <div className={`min-h-screen transition-colors duration-500 ${isDarkMode ? 'bg-[#05060a] text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <main className="pt-32 pb-20">
        <div className="max-w-7xl mx-auto px-6">
          {/* Hero Section */}
          <section className={`overflow-hidden rounded-[32px] border transition-all p-8 lg:p-12 mb-12 shadow-2xl relative ${
              isDarkMode 
            ...
            <div className="grid gap-12 lg:grid-cols-[1.4fr,0.8fr] lg:items-center relative z-10">
              <div>
                <button 
                  onClick={() => navigate('/explore-ideas')}
                  className="flex items-center gap-2 text-slate-500 hover:text-[#F24C20] mb-8 font-bold text-sm transition-colors group"
                >
                ...
                <div className="flex flex-wrap items-center gap-3 mb-6">
                  <div className="inline-flex items-center rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-2 text-xs font-bold uppercase tracking-widest text-emerald-400">
                    <ShieldCheck className="w-3.5 h-3.5 mr-2" /> {idea.status}
                  </div>
                  <div className="inline-flex items-center rounded-full border border-orange-500/30 bg-orange-500/10 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-orange-400">
                ...
                <p className={`text-lg md:text-xl leading-relaxed mb-8 max-w-2xl font-medium ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                ...
                {!idea.isUnlocked ? (
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        <button className="px-8 py-4 bg-[#F24C20] text-white rounded-2xl font-bold transition-all hover:bg-orange-600 shadow-xl shadow-[#F24C20]/20 hover:scale-105 active:scale-95">
                            Connect / Inquire Now
                        </button>
                        <button className={`px-8 py-4 rounded-2xl border font-bold transition-all hover:scale-105 active:scale-95 ${
                            isDarkMode 
              ...
              {/* Sidebar Preview */}
              <div className={`rounded-[40px] border p-8 shadow-2xl ${
                  isDarkMode 
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    {/* Developer Column */}
                    <div className={`rounded-3xl border p-4 transition-all overflow-hidden ${
                        isDarkMode 
                        ...
                        <div className="flex items-center gap-3">
                            {idea.creator?.profile_image ? (
                                <img src={idea.creator.profile_image} className="w-10 h-10 rounded-xl object-cover" />
                            ) : (
                                <div className="w-10 h-10 rounded-xl bg-[#F24C20]/10 flex items-center justify-center text-[#F24C20] text-sm font-black">
                                    {idea.creator?.full_name?.charAt(0)}
                                </div>
                            )}
                            <div className="min-w-0">
                                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 truncate">Expert</p>
                                <h4 className="font-black text-xs text-white truncate">{idea.creator?.full_name}</h4>
                            </div>
                        </div>
                    </div>
                  ...
                  <div className={`p-6 rounded-3xl bg-gradient-to-br from-[#F24C20] to-[#E23C10] text-white text-center shadow-lg shadow-orange-500/20`}>
                      <p className="text-[10px] font-bold uppercase tracking-widest opacity-80 mb-1">Estimated Valuation</p>
                      <p className="text-3xl font-black">{idea.fundingAmount || "Seed Stage"}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid gap-12 xl:grid-cols-[1.6fr,0.9fr]">
            <div className="space-y-12">
              <SectionCard title="Concept Overview" darkMode={isDarkMode}>
                <div className={`text-lg leading-relaxed whitespace-pre-wrap font-medium ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
              ...
              <SectionCard title="Roadmap & Sustainability" darkMode={isDarkMode}>
                <div className="space-y-8">
                  <div className="p-8 rounded-[32px] bg-gradient-to-br from-indigo-900/40 to-black/40 border border-indigo-500/30">
                     <p className="text-xs font-bold text-indigo-400 uppercase tracking-widest mb-4">Capital Allocation Model</p>
                     <p className={`text-sm leading-relaxed ${isDarkMode ? 'text-indigo-100' : 'text-slate-600'}`}>{idea.useOfFunds}</p>
                  </div>
...
function SectionCard({ title, children, darkMode }: any) {
  return (
    <section className={`rounded-[40px] border p-8 lg:p-10 transition-all ${
        darkMode 
...
function DetailItem({ label, value, icon, className = "", darkMode }: any) {
  return (
    <div className={`rounded-3xl border p-6 transition-all group ${
        darkMode 
      ...
      <div className={`text-base leading-relaxed font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>
        {value || "-"}
      </div>
    </div>
  );
...
function SummaryRow({ label, value, darkMode, highlight, color }: any) {
    return (
        <div className={`flex items-center justify-between py-4 border-b ${darkMode ? 'border-white/5' : 'border-gray-100'}`}>
            <span className="text-slate-500 uppercase text-[10px] font-bold tracking-widest">{label}</span>
            <span className={`text-sm font-black ${color ? color : (highlight ? 'text-[#F24C20]' : 'text-white')}`}>
                {value}
            </span>
        </div>
    );

...
export default function FreelancerDashboardHome() {
  ...
  // Gig Performance (Keep some fallbacks if performance object is empty)
  const gigPerformance = stats?.freelancer?.top_gigs || [
    { name: 'Logo Design Pro', orders: 45, revenue: 67500, rating: 4.9 },
    { name: 'Web Development', orders: 28, revenue: 84000, rating: 5.0 },
    { name: 'UI/UX Consultation', orders: 18, revenue: 36000, rating: 4.8 }
  ];

  // Completion Stats
  const performance = stats?.freelancer?.performance || {};
  const completionRate = performance.completion_rate || 100;
  const onTimeDelivery = performance.on_time_delivery || 100;
  const clientSatisfaction = performance.satisfaction ? (performance.satisfaction * 20) : 100; // Convert 5 star to %
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {/* Pending Payments (Hidden as platform is a bridge)
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-start justify-between mb-4">
            ...
            <Link to="/dashboard/balance" className="text-[#F24C20] text-sm font-medium hover:underline">
              View All
            </Link>
          </div>
          <div className={`text-sm ${isDarkMode ? 'text-neutral-400' : 'text-neutral-600'} mb-1`}>
          ...
          <div className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-neutral-900'}`}>
            <CountUp end={pendingPayments} prefix="₹" />
          </div>
        </motion.div>
        */}
        ...
        >
          ...
          <div className="space-y-6">
            <div className="flex flex-col items-center">
              <RadialProgress value={completionRate} color="#10b981" size={100} />
